...
if start <= 20 and end > 20:
    cmd = "python $TFNET_ROOT/scripts/prepare_data_pf.py " + tf + " " + cell_lines + " --no-bg > logs/pre_prepare.log 2>&1"
    logging.info(cmd)
    os.system(cmd)
if start <= 30 and end > 30:
    ...
    os.system("gunzip -c splits/test.tsv.gz | sort -k1,1 -k2,2n > subset_nobg.tsv")
...
if start <= 40 and end > 40:
    os.system("python $TFNET_ROOT/scripts/prepare_data_pf.py " + tf + " " + cell_lines + " > logs/prepare.log 2>&1")
